...
class GaussianSolidRigidGenerator : public SolidRigidGenerator
{
protected:
...
public:
	GaussianSolidRigidGenerator(PxScene* scene, PxPhysics* physics, std::string name, Vector3 std_dev_pos, Vector3 std_dev_vel, Vector3 mean_pos, Vector3 mean_vel, int n_particles) : SolidRigidGenerator(scene, physics), _std_dev_pos(std_dev_pos), _std_dev_vel(std_dev_vel), _mean_pos(mean_pos), _mean_vel(mean_vel) {
		_name = name;
