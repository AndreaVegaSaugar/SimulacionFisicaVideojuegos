...
public:
	...
	UniformParticleGenerator(PxScene* scene, PxPhysics* physics, std::string name, Vector3 pos_max, Vector3 pos_min, Vector3 vel_max, Vector3 vel_min, int n_solidRigids) : ParticleGenerator(scene, physics), _pos_max(pos_max), _vel_max(vel_max), _pos_min(pos_min), _vel_min(vel_min) {
		_name = name;
